        ...
        if (hid_keyboard_get_char(key_event->modifier,
            ...
            if (KEYBOARD_ENTER_MAIN_CHAR == key_char) {
                ...
                if (!timer_created) {
                    ...
                    ESP_ERROR_CHECK(esp_timer_create(&timer_args, &test_timer));
                    timer_created = true;
                    ESP_LOGI(TAG, "test_timer created");
                }

                ESP_ERROR_CHECK(esp_timer_start_periodic(test_timer, 800 * 1000));
                ESP_LOGI(TAG, "test_timer started periodic (800ms)");
            }
        }
    }
}
